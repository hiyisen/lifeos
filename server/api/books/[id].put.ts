...
export default defineEventHandler(async (event) => {
  ...
  db.run(
    `UPDATE books SET title=?, author=?, isbn=?, type=?, rating=?, review=?, cover_path=?, source_id=?, source_url=?, status=?, page_count=?, current_page=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
    body.title,
    body.author || null,
    body.isbn || null,
    body.type || 'paper',
