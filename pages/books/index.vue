...
const { items: books, loading, initialLoading, hasMore, sort, setSort, sentinel, fetch, reset } =
  useInfiniteList({
...
<template>
  <div>
    ...
    <div ref="sentinel" class="flex justify-center py-8">
      ...
      />
      <span v-else-if="!hasMore && books.length > 0" class="text-xs text-[var(--color-text-secondary)]">已加载全部</span>
    </div>
