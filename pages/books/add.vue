...
async function onSubmit(data: Record<string, any>) {
  ...
  try {
    ...
    if (res.success) navigateTo(`/books/${res.data.id}`);
...
<template>
  <div class="mx-auto max-w-xl">
    <h2 class="mb-6 text-lg font-bold text-[var(--color-text)]">添加书籍</h2>
    <div
      v-if="error"
    ...
    <BookForm :loading="loading" @submit="onSubmit" @cancel="navigateTo('/books')" />
  </div>
</template>
