...
async function onSubmit(data: Record<string, any>) {
  ...
  try {
    ...
    if (res.success) navigateTo(`/games/${res.data.id}`);
...
<template>
  <div class="mx-auto max-w-xl">
    <h2 class="mb-6 text-lg font-bold text-[var(--color-text)]">添加游戏</h2>
    <div
      v-if="error"
    ...
    <GameForm :loading="loading" @submit="onSubmit" @cancel="navigateTo('/games')" />
  </div>
</template>
