...
async function onSubmit(data: Record<string, any>) {
  ...
  error.value = '';
  try {
    const res = await api.put(`/api/games/${route.params.id}`, data);
    if (res.success) navigateTo(`/games/${route.params.id}`);
...
<template>
  <div class="mx-auto max-w-xl">
    <h2 class="mb-6 text-lg font-bold text-[var(--color-text)]">编辑游戏</h2>
    <div
      v-if="error"
    ...
    <GameForm
      ...
      is-edit
      @submit="onSubmit"
      @cancel="navigateTo(`/games/${route.params.id}`)"
    />
  </div>
</template>
